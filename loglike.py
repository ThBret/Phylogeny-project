    ...
    if not os.path.exists(path_nd):
        ...
        for gen in genera_or:
            with open(path_or + gen) as fp:
                ...
                for entry in contents.splitlines():
                    if 'Log-likelihood of the tree' in entry:
                        loglikes_or[gen.replace('.fas.iqtree','')] = entry.split(' ')[4]
        ...
        #Number of spp
        spp_gen_all = pd.read_excel(p.replace('M1/' + sys.argv[1] + '/','') + 'Sheets/log_' + sys.argv[1] + '.xlsx', usecols=[0,1,2,3,4])
...
lr_df_total.to_excel(p.replace('M1/' + sys.argv[1] + '/','') + 'Sheets/' + 'lr_df_log_' + sys.argv[1] + '_M1.xlsx', index = False)