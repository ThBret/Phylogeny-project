...
os.chdir(path + '_iq')
for t in sorted([f for f in os.listdir(path + '_iq/') if not f.startswith('.') if f.endswith('.cropped_tree')]):
	if os.path.getsize(t):
		tree = Phylo.read(t, "newick")
		tabulate_names(tree)
		clades = get_internodes(tree)
		Phylo.draw_ascii(tree)
		os.mkdir(path + '_node_trees/' + t.replace('.fas.cropped_tree','') + '_node_trees')
		for i in range(len(clades)):
			tree.collapse(target=clades[i])
			Phylo.write(tree, path + '_node_trees/' + t.replace('.fas.cropped_tree','') + '_node_trees/' + str(i) + ".nwk", "newick")
			Phylo.draw_ascii(tree)
			tree = Phylo.read(t, "newick")
			tabulate_names(tree)
			clades = get_internodes(tree)
